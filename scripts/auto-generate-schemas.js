...
async function testConnection(credentialFile) {
  try {
    ...
    console.warn(`Connection test failed for ${credentialFile}:`, error.message);
    return false;
  }
}
...
async function generateQueryBuilder(credentialFile, instanceName) {
  try {
    console.log(`Generating EdgeQL-JS query builder for instance: ${instanceName}`);
    ...
    console.log(`✅ Query builder generated for ${instanceName} in ${outputDir}`);
    return true;
  } catch (error) {
    console.error(`❌ Failed to generate query builder for ${instanceName}:`, error.message);
    ...
    try {
      ...
      console.warn('Failed to cleanup after generation failure:', cleanupError.message);
    }
...
export { default } from './${defaultInstance}';`;
    ...
    console.log(`✅ Created query builder index with ${instanceDirs.length} instance(s)`);
...
async function main() {
  try {
    ...
    if (!fs.existsSync(credentialsDir)) {
      console.log('No instance_credentials directory found. Creating fallback structure.');
    ...
    console.log(`Found ${jsonFiles.length} credential file(s): ${jsonFiles.join(', ')}`);
    ...
    for (const file of jsonFiles) {
      ...
      if (canConnect) {
        ...
        const generated = await generateQueryBuilder(credentialFile, instanceName);
        if (generated) {
    ...
    if (results.successful.length > 0) {
      console.log(`✅ Successfully generated for: ${results.successful.join(', ')}`);
    }
