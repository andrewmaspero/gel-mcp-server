...
export function registerSearchDocs(server: McpServer) {
  server.registerTool(
    'search_gel_docs',
    {
      ...
      description: 'Search the Gel documentation for specific terms or patterns.',
      inputSchema: {
    ...
    async (args) => {
      ...
      if (!docFilePath) {
        return { content: [{ type: 'text', text: `Documentation file not found. Searched paths: ${possiblePaths.join(', ')}` }] };
      }
      ...
      try {
        ...
        const chunks: Array<{id: number, content: string, startLine: number, endLine: number}> = [];
        ...
        if (results.length === 0) {
          return { content: [{ type: 'text', text: `No matches found for "${args.search_term}".` }] };
        }
        ...
        return { content: [{ type: 'text', text: `Error reading documentation: ${error.message}` }] };
      }
    }
  );
}
