		...
		async (args) => {
			...
			try {
				switch (action) {
					...
					case "listBranches": {
						const instance =
							args.instance || getDefaultConnection().defaultInstance;
						if (!instance) {
							return buildToolResponse({
								status: "error",
						...
						for (const line of lines) {
							...
							if (
								!t ||
								t.startsWith("Available branches") ||
								t.startsWith("---")
							)
								continue;
							...
							if (currentMatch)
								...
								branches.push({ name: t, current: false });
						}
					...
					case "set": {
						...
						const pickedInstance =
							args.instance ?? getAvailableInstances().sort()[0];
						if (!pickedInstance) {
							return buildToolResponse({
								status: "warn",
